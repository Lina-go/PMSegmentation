...
from Models.networks import U_Net, R2U_Net, AttU_Net, R2AttU_Net
import os 
...
# Initialize WandB
wandb.login()
run_name = f"{args.model_type}_bs{args.batch_size}_lr{args.lr}"
...
def test(epoch, best_dice, model):
    ...
    if Dice > best_dice:
        best_dice = Dice
        torch.save(model.state_dict(), f"{args.model}_{args.model_type}_best.pth")

    with open(f"dice_{args.model_type}.txt", "a") as f:
        f.write(f"Test\n")
